#include <memory>
#include <vector>
#include <iostream>
#include <variant>
#include <map>
#include "msglib.h"
//#1 使用std::variant
// 问题 std::::variant的大小是最大的结构体最大的大小+unsigned int， 带来类型膨胀
// 性能开销，不支持空值。
//using Msg = std::variant<MoveMsg, JumpMsg, SleepMsg, ExitMsg>;


//#2 自定义一个虚函数
struct MsgBase
{
    virtual void speak() = 0;
    virtual void happy() = 0;
    virtual std::shared_ptr<MsgBase> clone() const = 0;  // 拷贝构造
    virtual ~MsgBase() = default;

    using Ptr = std::shared_ptr<MsgBase>;
};

template<class Msg>
struct MsgImpl:MsgBase
{
    Msg msg;

    template<class ...Ts>
    MsgImpl(Ts &&...ts):msg{std::forward<Ts>(ts)...} { }

    std::shared_ptr<MsgBase> clone() const override {
        return std::make_shared<MsgImpl<Msg>>(msg);
    }

    void speak() override {
        msg.speak();
    }

    void happy() override {
        if constexpr ( requires {
            msg.happy();
        }) {
            msg.happy();
        } else {
            std::cout<< "no happy\n";
        }
    }
};

// 工厂模板函数
struct MsgFactoryBase {
    virtual MsgBase::Ptr create() = 0;
    virtual ~MsgFactoryBase() = default;

    using Ptr = std::shared_ptr<MsgFactoryBase>;
};

template<class Msg>
struct MsgFactoryImp :public MsgFactoryBase{
    MsgBase::Ptr create() override {
        return std::make_shared<MsgImpl<Msg>>();
    }
};

template<class Msg>
MsgFactoryBase::Ptr makeFactory() {
    return std::make_shared<MsgFactoryImp<Msg>>();
}




template<class Msg, class ...Ts>
std::shared_ptr<MsgBase> makeMsg(Ts &&...ts) {
    return std::make_shared<MsgImpl<Msg>>(std::forward<Ts>(ts)...);
}

std::vector<std::shared_ptr<MsgBase>> msgs;


//#3 用户输入
struct TestClient {
    inline static const std::map<std::string, MsgFactoryBase::Ptr> factories = {
        {"Move",  makeFactory<MoveMsg>()},
        {"Jmp",   makeFactory<JumpMsg>()},
        {"Sleep", makeFactory<SleepMsg>()},
        {"Exit",  makeFactory<ExitMsg>()},
    };
    void reve_data() {
        std::string type;
        std::cin>> type; // 用户输入

        factories.at(type)->create();
        // if (type == "Move") {
        //     // is_mov = true;
        //     // mov = std::make_shared<MoveMsg>(1, 2);
        //     msg = makeMsg<MoveMsg> (1,2);
        // } 
        // if (type == "Jump") {
        //     // is_jmp = true;
        //     // jmp = std::make_shared<JumpMsg>(5);
        //     msg = makeMsg<JumpMsg> (5);
        // }
        // if (type == "Sleep") {
        //     // is_slp = true;
        //     // slp = std::make_shared<SleepMsg>(1);
        //     msg = makeMsg<SleepMsg> (1);
        // } 
        // if(type == "Exit") {
        //     // is_ext = true;
        //     // ext = std::make_shared<ExitMsg>();
        //     msg = makeMsg<ExitMsg> ();
        // }
    }

    void update() {
        // if (is_mov) {
        //     mov->speak();
        // }
        // if (is_jmp) {
        //     jmp->speak();
        // }
        // if (is_slp) {
        //     slp->speak();
        // }
        // if (is_ext) {
        //     ext->speak();
        // }
        if (msg) {
            msg->speak();
        }

    }

    //#1
    // std::shared_ptr<MoveMsg> mov;
    // std::shared_ptr<JumpMsg> jmp;
    // std::shared_ptr<SleepMsg> slp;
    // std::shared_ptr<ExitMsg> ext;

    // bool is_mov;
    // bool is_jmp;
    // bool is_slp;
    // bool is_ext;

    //#2 
    MsgBase::Ptr msg;
};

int main() {

    msgs.push_back(makeMsg<MoveMsg>(5, 10));
    msgs.push_back(makeMsg<JumpMsg>(20));
    msgs.push_back(makeMsg<SleepMsg>(8));
    msgs.push_back(makeMsg<ExitMsg>());

    for (auto &msg:msgs) {
        msg->speak();
        msg->happy();
    }

    return 0;
}